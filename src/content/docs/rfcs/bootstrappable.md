...
- Bootstrapping an existing distribution that is already bootstrappable up to a certain point until glaucus's toolchain is able to take over would enable for an interesting combination at least early on (e.g. StageX); that is ofc until glaucus is fully bootstrappable from absolute nothingness
