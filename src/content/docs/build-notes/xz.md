...
- `xz` can also decompress `lzip` files (`.lz`), as they share the same format (LZMA), starting from `5.4.0`; `tarlz` is faster than `lzip` in compression but is equal in decompression
