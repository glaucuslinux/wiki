...
- skarnet `.pc` files include traces from the host glaucus directory which does not exist in `chroot`
...
- `s6` is now capable of emulating daemontools and runit programs
- `--prefix` is ignored when `--enable-slashpackage` is used
