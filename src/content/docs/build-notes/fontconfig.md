...
## Prepare
- Check if `NOCONFIGURE=yes ./autogen.sh` works compared to `autoreconf -vfis`; the latter works with `gettext-tiny`

- Build against `freetype-harfbuzz` and not `freetype` alone
- Depends on libpng (optional), bzip2, zlib, harfbuzz (optional, circular, adds glib2 and pcre)?
...
- Is `--disable-static` set by default?
...
```
- run the following pre/post install:
```
fc-cache -vf
